...
namespace com.amtec.action
{
    public class GetWorkOrder
    {
        ...
        public string[] GetPartNumberByWorkOrder(string woFilter)
        {
            KeyValue[] workorderFilter = new KeyValue[] {new KeyValue("WORKORDER_NUMBER", woFilter) };
            string[] mdataGetWorkordersResultKeys = new string[] { "PART_NUMBER","WORKORDER_STATE"};//F = opened (released);S = started
            string[] mdataGetWorkordersResultValues = new string[] { };
            bool isHasmode = true;
            int error = imsapi.mdataGetWorkorders(sessionContext, init.configHandler.StationNumber, workorderFilter, mdataGetWorkordersResultKeys, out mdataGetWorkordersResultValues,out isHasmode);
            LogHelper.Info("begin api mdataGetWorkorders (Station number:" + init.configHandler.StationNumber + ",workorder filter:" + woFilter + "result code = " + error + ")"); LogHelper.Info("begin api mdataGetWorkorders (Station number:" + init.configHandler.StationNumber + ",workorder filter:" + woFilter + "result code = " + error + ")");
            return mdataGetWorkordersResultValues;
        }
