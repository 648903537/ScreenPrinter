...
namespace com.amtec.action
{
    class SocketClientHandler2
    {
        ...
        public void send(string data)
        {
            ...
            Bysend = Encoding.UTF8.GetBytes(data);//Encoding.GetEncoding("UTF-8").GetBytes(data); //将字符串指定到指定Byte数组
        ...
        public string SendData(string message)
        {
            try
            {
                ...
                string str = Encoding.UTF8.GetString(back, 0, count);//.GetEncoding("UTF-8").GetString(back, 0, count); //Encoding.Default.GetString(back, 0, count);
