...
namespace com.amtec.action
{
    public class UploadProcessResult
    {
        ...
        public int ScrapProcessResultCall(string serialNumber, string station)
        {
            String[] serialNumberUploadKey = new String[] { "ERROR_CODE", "SERIAL_NUMBER", "SERIAL_NUMBER_POS", "SERIAL_NUMBER_STATE" };
            String[] serialNumberUploadValues = new String[] { };
            String[] serialNumberResultValues = new String[] { };
            error = imsapi.trUploadState(sessionContext, station, init.currentSettings.processLayer, serialNumber, "-1", 2, 1, -1, 0, serialNumberUploadKey, serialNumberUploadValues, out serialNumberResultValues);
            LogHelper.Info("Api trUploadState: serial number =" + serialNumber + ", result code =" + error);
            if ((error != 0) && (error != 210))
            {
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " trUploadState " + error, "");
        ...
        public int UploadPreProcessResultCall(string serialNumber, string station, long bookdate)
        {
            String[] serialNumberUploadKey = new String[] { "ERROR_CODE", "SERIAL_NUMBER", "SERIAL_NUMBER_POS", "SERIAL_NUMBER_STATE" };
            String[] serialNumberUploadValues = new String[] { };
            String[] serialNumberResultValues = new String[] { };
            error = imsapi.trUploadState(sessionContext, station, init.currentSettings.processLayer, serialNumber, "-1", 0, 1, bookdate, 0, serialNumberUploadKey, serialNumberUploadValues, out serialNumberResultValues);
            LogHelper.Info("Api trUploadState: station+" + station + ",serial number =" + serialNumber + ", result code =" + error);
            if ((error != 0) && (error != 210))
            {
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " trUploadState " + error, "");
                return error;
